...
UNITY_INTERFACE_EXPORT void UNITY_INTERFACE_API LoadRaw(unsigned int id, const void *pData, size_t dataSize, int width, int height)
{
   if (auto *pLoader = GetLoader(id))
   {
      pLoader->LoadRaw(pData, dataSize, width, height);
   }
}
